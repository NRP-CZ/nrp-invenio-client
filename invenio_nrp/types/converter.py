...
def extend_serialization(
    *modifications: Omit | Rename | WrapStructure | WrapUnstructure,
    allow_extra_data: bool = False,
) -> Callable[[type], type]:
    """Add serialization rules to an attrs class."""

    def wrapper[T](t: type[T]) -> type[T]:
        """Wrap the class and register type for that."""
        th = TypeHookBuilder(t, allow_extra_data)
        for modification in modifications:
            match modification:
                case Omit(field, from_structure, from_unstructure):
                    if from_structure:
                        th.omit_from_structure(field)
                    if from_unstructure:
                        th.omit_from_unstructure(field)
                case Rename(serialized_name, model_name):
                    th.rename_field(serialized_name, model_name)
                case WrapStructure(wrapper):
                    th.add_structure_wrapper(wrapper)
                case WrapUnstructure(wrapper):
                    th.add_unstructure_wrapper(wrapper)
        th.build_type_hook()
        return t
