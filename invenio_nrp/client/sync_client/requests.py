...
def single_value_expected(value: dict | list) -> None:
    """Check that the value is a dictionary/list with exactly one key."""
...
class RequestPayload(Model):
    ...
    @classmethod
    def __pydantic_restore_hierarchy(cls, data: Any) -> Any:  # noqa
        if not data:
            return {}
        obj = {}
    ...
    @classmethod
    def _parse_colon_hierarchy(self, obj: dict, key: str, value: Any) -> None:  # noqa ANN401
        parts = key.split(":")
        for part in parts[:-1]:
...
class RequestClient[RequestBase: Request]:
    ...
    def all(self, **params: str) -> RequestList[RequestBase]:
