...
class Downloader:
    ...
    The downloader can be used to download files from the NRP Invenio API or other https sources.
    """
    ...
    async def __aenter__(self) -> Self:
        """Start the downloader when context is started."""
        # start the job group
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:  # noqa
        """Stop the downloader when context is exited."""
        self.progress.downloader_before_finish(self)
        try:
            assert self._tg is not None, "Downloader not started"
            await self._tg.__aexit__(exc_type, exc_val, exc_tb)
            return None
        finally:
            self.progress.downloader_finished(self)

    def add(self, url: str, sink: DataSink, auth: Optional[Any] = None) -> None:  # noqa: ANN401
    ...
    async def _start_download(
        ...
        url: str,
        sink: DataSink,
        auth: Optional[Any] = None,  # noqa: ANN401
    ) -> None:
        """Start the download, guarded by the limiter.
    ...
    def create_task(self, task: Any) -> asyncio.Task:  # noqa: ANN401
        """Create a new task, called from job & chunk.
    ...
    async def stop(self) -> None:
        """Abort all downloads."""
        if self._tg:
            await self._tg._abort()  # type: ignore
